...
//		String sql = "select * from product where name like '%캔%'";
		...
		// 결과 집합은 여러 줄(row)로 구성되어 있다. Scanner와 마찬가지로 다음 줄이 있으면 반복한다
		while (rs.next()) {
			int idx = rs.getInt("idx");			// idx컬럼의 값을 정수 형태로 가져온다
			String name = rs.getString("name");	// name컬럼의 값을 문자열 형태로 가져온다
			int price = rs.getInt("price");		// price컬럼의 값을 정수 형태로 가져온다
		...
		// 작업이 끝났으면, 연결할 때의 역순으로 객체를 닫아준다
		rs.close();		// close()를 제대로 수행하지 않으면
		stmt.close();	// DB의 최대 접속인원수를 초과하여
		conn.close();	// 다음 실행 때 접속이 되지 않아서, DB서비스를 재시작해야 한다
