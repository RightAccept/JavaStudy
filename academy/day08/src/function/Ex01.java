...
public class Ex01 {
	// 함수 정의 (어떤 형식으로 실행할지 결정하고, 함수의 이름과 형태를 지정한다)
	// 정의하는 시점에서 실행되는 것은 아니다
	static int add(int n1, int n2) {		// 인자를 매개변수에 받아서
		int answer = n1 + n2;				// 매개변수의 값을 이용하여 코드를 진행한
		return answer;						// 마지막에 결과값을 반환한다
	}										// 반환값은 함수 호출자리로 되돌아가게 되어 있다
											// 함수의 반환값은 곧 함수 그 자체의 자료형이 된다

	public static void main(String[] args) {
		// 함수 : 자주 사용되는 코드를 묶어서 필요할 때마다 실행할 수 있는 형태로 지정한 형식
